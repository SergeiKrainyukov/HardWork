...
```java
 private void sortDataByApartmentV2() {
        data.sort((com1, com2) -> {
        Tag tag1 = com1.getTask().getTags().stream().filter(tag -> tag.isOptimalFilter()).findFirst().orElse(null);
        Tag tag2 = com2.getTask().getTags().stream().filter(tag -> tag.isOptimalFilter()).findFirst().orElse(null);
